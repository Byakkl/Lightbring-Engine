...
public:
    ...
    std::optional<Handle<Mesh>> importMesh(const char*, bool = true);
    ...
    /// @return Returns a pointer to the imported data structure if successful. Nullptr if not
    std::optional<Handle<Texture>> importImage(const char*, bool = true);
    ...
    void setCameraActive(Comp_Camera*, bool);
